...
def _knowledge_reminder(task: PublicTask) -> str:
    ...
    return (
        ...
        "do not define the needed rule, coded value, or ambiguous term. "
...
def build_task_prompt(task: PublicTask, *, codeact: bool = False) -> str:
    ...
    if codeact:
        return (
            ...
            "Before finalizing, verify that the row set, row grain, and final columns exactly match the question."
        )
    return (
        f"Question: {task.question}\n"
        f"{context_manifest}\n"
